...
char* getInput(){
	char *inc;
	int i=0;
	inc=malloc(1000*sizeof(char));
	char p[1];
	readc=recv(connectfd,&p[0],1,0);
	inc[i]=p[0];
	i++;
	...
	printf("(debug) : Recieved %d bytes total.\n", i);
	printf("%s",inc);
	return inc;
}
void putOut(char *arg){
	int putout;
	printf("%s", arg);
	putout=write(connectfd,arg,strlen(arg));
			...
			{	
				...
				else if(message[0]=='C')
				{
					...
					else
					{
						...
						if(debug) printf("(debug): Sending Server Response: %s",confirm);
						putOut(confirm);
						if(debug) printf("(debug): Sent Server Response: %s",confirm);
					}
