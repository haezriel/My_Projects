#include <stdio.h>
#include <errno.h>
#include <string.h>
#include <unistd.h>
#include <fcntl.h>
#include <stdlib.h>
#include <time.h>
#include <stdint.h>
#include <inttypes.h>


void makekey();
void getkey();
unsigned char gethex(char buf);
uint16_t leftrotate(uint16_t in);
uint16_t rightrotate(uint16_t in);
void F(uint16_t R0, uint16_t R1, int round);
uint8_t K(int x);
uint16_t G(uint16_t R, int round, uint8_t subkey[]);
uint16_t f[2];
uint8_t key[8];
uint64_t big_key;
uint64_t word_block;
int debug=0;
unsigned char ftable[]=
{};

int main(int argc, char *argv []){
	char k;
if(argc==2 ){
	sscanf(argv[1],"%c",&k);
if(k=='d'){
	printf("Debugging Mode On\n");
	printf("Printing to output.txt.\n");
	debug=1;
}
}
int round;
printf("Decrypting, printing to plaintext.txt.\n");
FILE *fp;
FILE *fp2;

fp=fopen("cyphertext.txt","r+");
if(fp==NULL){
	printf("No cyphertext.txt file found.\n");
	exit(1);
}
if(debug==0)
	fp2=fopen("plaintext.txt","w+");
else
	fp2=fopen("output.txt","w+");
size_t bsize;
uint64_t block=0;
bsize=fread(&block, sizeof(block), 1, fp);
while(1){

if(bsize!=1){
	fclose(fp);
	fclose(fp2);
	exit(0);
}
uint16_t W[4];
uint16_t R[4];
getkey();
if(debug==1)
printf("key: %"PRIx64"\n",big_key);

W[0]=block>>48&0xffff;
W[1]=block>>32&0xffff;
W[2]=block>>16&0xffff;
W[3]=block&0xffff;


for (int i=0;i<4;i++){
	uint16_t key16=key[i*2]<<8|key[i*2+1];
	R[i]=W[i]^key16;
	if(debug==1)
	printf("%x",R[i]);
}
if(debug==1)
printf("\nblock whitening complete.\n");
W[1]=rightrotate(W[1]);
for(round=0;round<16;round++){
	uint16_t interm[4];
	f[0]=0;
	f[1]=0;
	F(R[0],R[1],round);
	interm[0]=R[2];
	interm[0]=leftrotate(interm[0]);
	interm[0]=interm[0]^f[0];
	interm[1]=R[3]^f[1];
	interm[1]=rightrotate(interm[1]);
	interm[2]=R[0];
	interm[3]=R[1];
	R[0]=interm[0];
	R[1]=interm[1];
	R[2]=interm[2];
	R[3]=interm[3];	
	if(debug==1)
	printf("%x%x%x%x\n",R[0],R[1],R[2],R[3]);
}
//printf("%"PRIx64"\n",big_key);

uint16_t y[4];
y[0]=R[2];
y[1]=R[3];
y[2]=R[0];
y[3]=R[1];

uint64_t C[4];

for (int i=0;i<4;i++){
	uint16_t key16=big_key>>(3-i)*16&0xffff;
	C[i]=y[i]^key16;

}

uint64_t cyphertext=C[0]<<48|C[1]<<32|C[2]<<16|C[3];


block=0;
bsize=fread(&block, sizeof(block), 1, fp);
if(bsize!=1){
	uint8_t brick=cyphertext&0xff;
	cyphertext=cyphertext>>8;
		fwrite(&cyphertext, sizeof(uint8_t)*brick, 1, fp2);
	fclose(fp);
	fclose(fp2);
	exit(0);
}


	fwrite(&cyphertext, sizeof(cyphertext), 1, fp2);

}




}

void makekey(){
FILE *fp;
time_t t;
srand((unsigned) time(&t));
fp=fopen("key.txt","w+");


for(int i=0;i<8;i++){
key[i]=rand()%256;
big_key=big_key|key[i];
if(i<7)big_key=big_key<<8;
}
fwrite(&big_key,sizeof(big_key),1,fp);
fclose(fp);
return;
}

void getkey(){
FILE *fp;
fp=fopen("key.txt","r+");
size_t size;
if((size=fread(&big_key,sizeof(big_key),1,fp))!=1){
	printf("Bad Key\n");
	exit(1);
}

for(int i=0;i<8;i++){
	key[7-i]=big_key>>8*i&0xff;
}
fclose(fp);
	//printf("%"PRIx64"\n",big_key);
return;
}

void F(uint16_t R0, uint16_t R1, int round){
	uint8_t subkey[12];
	for(int i=0;i<12;i++){
		subkey[11-i]=K(4*round+(i%4));
	}
	for(int i =0;i<12;i++){
		if(debug==1)
		printf("Round %d, subkey#%d=%x \n",round, i, subkey[i]);}
	uint8_t gkeys1[4];
	uint8_t gkeys2[4];
	for(int i=0;i<4;i++){
		gkeys1[i]=subkey[i];
		gkeys2[i]=subkey[i+4];
	}
	uint16_t T0=G(R0,round,gkeys1);
	uint16_t T1=G(R1,round,gkeys2);
	f[0]=(T0+2*T1+(subkey[8]<<8|subkey[9]))%65536;
	f[1]=(2*T0+T1+(subkey[10]<<8|subkey[11]))%65536;
	if(debug==1)
	printf("T0: %x T1: %x f0: %x f1: %x\n",T0,T1,f[0],f[1]);

}

uint8_t K(int x){
	for(int i=0;i<8;i++){
	key[7-i]=big_key>>(8*i)&0xff;
	//printf("key #%d, %x\n",i,key[i]);
}
//printf("%"PRIx64"\n",big_key);
uint64_t big_key_shift=big_key&0x1;
big_key=big_key_shift<<63|big_key>>1;


return key[x%8];

}
uint16_t G(uint16_t R,int round,uint8_t subkey[]){
uint8_t g1=R>>8&0xff;
uint8_t g2=R&0xff;
uint8_t g3=ftable[(g2^subkey[0])]^g1;
uint8_t g4=ftable[(g3^subkey[1])]^g2;
uint16_t g5=ftable[(g4^subkey[2])]^g3;
uint16_t g6=ftable[(g5^subkey[3])]^g4;
if(debug==1)
printf("g1: %x g2: %x g3: %x g4: %x g5: %x g6: %x\n",g1,g2,g3,g4,g5,g6);
return (g5<<8|g6);
}
uint16_t rightrotate(uint16_t in){
uint16_t shift=in& 0x0001;
in=shift<<15|in>>1;
return in;
}
uint16_t leftrotate(uint16_t in){
uint16_t shift=in& 0x8000;
in=shift>>15|in<<1;
return in;
}
