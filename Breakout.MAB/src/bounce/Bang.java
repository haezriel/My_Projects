 ...
 * explosions to determine when they are no longer active and remove/hide them
 * at that point.
 */
class Bang extends Entity {
	...
	public Bang(final float x, final float y) {
		super(x, y);
		explosion = new Animation(ResourceManager.getSpriteSheet(BounceGame.BANG_EXPLOSIONIMG_RSC, 64, 64), 0, 0, 22, 0,
				true, 50, true);
	...
	public Bang(final float x, final float y, boolean cena) {
		super(x, y);
		explosion = new Animation(ResourceManager.getSpriteSheet(BounceGame.BANG_EXPLOSIONIMG_RSC, 64, 64), 0, 0, 22, 0,
				true, 50, true);
		addAnimation(explosion);
